...
class NewsRepositoryImplTest {
	...
	private val newsRepositoryImpl = NewsRepositoryImpl(localNewsDataSource, remoteNewsDataSource, scope)

	@Test
	fun testNewsRepo() = runTest {
		...
		whenever(localNewsDataSource.fetchSubscribedCategories()).thenReturn(flowOf(CategoryModel.entries))

		val trendingNews = newsRepositoryImpl.fetchTrendingNews("", "", "").first()
		val sourceDetails = newsRepositoryImpl.fetchSourceDetails(sampleNewsSource).first()

		val fetchedData = listOf(trendingNews, sourceDetails)
		val sampleData = listOf(sampleNewsData, sampleNewsSourceDetail)
		assertEquals(fetchedData, sampleData)
	}
}